...
(`window_kg_lease`): `owner`, `serving_*`, `prev_*` (copy of serving at steal).

- Steal: `prev ← serving`, `SET owner IF owner = previous`. Serving unchanged.
- Publish (timer / OnCommit / CP, one function): if `current_wm >= serving_wm`,
  `SET serving_* IF owner = me`.
...
| 3 | Scylla write path | `feat/scylla-wo-write` ([#287](https://github.com/volga-project/volga/pull/287)) | Session, DDL, raw / tiles / `key_states` / **versioned triggers**, `commit_events` (same-partition UNLOGGED BATCH). WO overlay `me \| (cp.attempt ∧ E ≤ cp_E)` once a checkpoint exists; until then `me` only. `load_triggers` (`LIMIT` + last-raw clustering seek). Derive `key_group`. **No lease, no `window_head`, no ingest LWT.** Testcontainer ingest + load + emit. |
| 4 | Checkpoint / restore + request lease | `feat/scylla-wo-checkpoint` ([#288](https://github.com/volga-project/volga/pull/288)) | `Versioned { version }`; **continue `E` (never 0)**. Restore writer from CP into in-memory overlay slices. Request: `window_kg_lease` per `key_group`; steal copies `prev ← serving`; `publish()` after `serving_wm` (`on_commit` / `interval` / `checkpoint` = who calls it). Identity `RestorePlanner`. **Rewrite:** drop `window_recovery_bases`, HeadClaim / DashMaps, per-key head, ingest-path catch-up freeze. Existing checkpoint e2e + Scylla. |
| 5 | `maintain` / GC | `feat/scylla-wo-maintain` ([#289](https://github.com/volga-project/volga/pull/289)) | `window_kg_buckets`, `floor_bucket`, raw + all tile granularities. Drop `prev.attempt ∧ E > prev.E`. Do **not** drop unpublished current-writer `E > serving_E` or `E ≤ serving_E` holes until time GC / compact. Serving-pin grace. |
| 6 | Scylla request store | `feat/scylla-request-store` ([#290](https://github.com/volga-project/volga/pull/290)) | Pin `window_kg_lease` once; client-side WRO filter + per-cell `max(E)` then `max(attempt)`. Register `backend = Scylla` on the PR 2 runner. |
