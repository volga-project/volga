...
| E `volga-longrun` + both scenarios | 4+5 | Longrun | not started |
| F Prom + Grafana + dashboard JSON | 6 | Observability | not started |
| G dump + oracles | 7 | Longrun | not started |
| H operator-owned InMemory store | — | Operator | not started |

A, B, C, H have no mutual dependency — land whenever. Stack longrun **D → E → G**. F needs A + C, not the binary. E is simpler if **H** (or at least “do not rewrite sink to InMemory”) has landed.

Do not merge: D with B; B with A; G with E.
...
- Datagen + `run_for_s`. `SinkSpec::Count`. Sliding RANGE window SQL on `PipelineSpec` (source → query → sink). Default: existing 10s checkpoint window SQL in `src/tests/support/checkpoint/launch.rs`.
...
| Lag p99 | `histogram_quantile(0.99) > 15s` for a sustained 60s | Tighten on a real cluster |
...
| **G** dump + oracles | 7 | Longrun | **E** (run loop). Ideally **F** (same series as the board). |
