...
pub fn print_pipeline_state(
    ...
    let throughput_rates: Option<HashMap<String, ThroughputRates>> =
        if let (Some(hist), Some(window_secs)) = (history, throughput_window_seconds) {
    ...
    let should_include_task = |vertex_id: &str| -> bool {
        if let Some(operator_ids) = operator_ids {
            operator_ids
                .iter()
                .any(|op_id| vertex_id.starts_with(op_id))
        } else {
            true
        }
    };
            ...
            if !tasks_only {
                ...
                for (operator_id, operator_metrics) in &worker_metrics.operator_metrics {
                    if should_include_operator(operator_id) {
                        ...
                        println!(
                            "      Messages Sent: {} (total)",
                            operator_metrics.throughput_metrics.messages_sent
                        );
                        println!(
                            "      Messages Recv: {} (total)",
                            operator_metrics.throughput_metrics.messages_recv
                        );
                        println!(
                            "      Records Sent: {} (total)",
                            operator_metrics.throughput_metrics.records_sent
                        );
                        println!(
                            "      Records Recv: {} (total)",
                            operator_metrics.throughput_metrics.records_recv
                        );
                        println!(
                            "      Bytes Sent: {} (total)",
                            operator_metrics.throughput_metrics.bytes_sent
                        );
                        println!(
                            "      Bytes Recv: {} (total)",
                            operator_metrics.throughput_metrics.bytes_recv
                        );
                        println!("    Latency:");
            ...
            if !operators_only {
                ...
                for (vertex_id, task_metrics) in &worker_metrics.tasks_metrics {
                    if should_include_task(vertex_id) {
                        ...
                        println!(
                            "      Messages Sent: {} (total)",
                            task_metrics.throughput_stast.messages_sent
                        );
                        println!(
                            "      Messages Recv: {} (total)",
                            task_metrics.throughput_stast.messages_recv
                        );
                        println!(
                            "      Records Sent: {} (total)",
                            task_metrics.throughput_stast.records_sent
                        );
                        println!(
                            "      Records Recv: {} (total)",
                            task_metrics.throughput_stast.records_recv
                        );
                        println!(
                            "      Bytes Sent: {} (total)",
                            task_metrics.throughput_stast.bytes_sent
                        );
                        println!(
                            "      Bytes Recv: {} (total)",
                            task_metrics.throughput_stast.bytes_recv
                        );
                        ...
                        if let Some(ref rates) = throughput_rates {
                            if let Some(rate) = rates.get(vertex_id) {
                                println!(
                                    "    Throughput Rates (avg over {}s window):",
                                    throughput_window_seconds.unwrap_or(0)
                                );
                                println!(
                                    "      Messages Sent: {:.2}/s",
                                    rate.messages_sent_per_sec
                                );
                                println!(
                                    "      Messages Recv: {:.2}/s",
                                    rate.messages_recv_per_sec
                                );
...
pub fn verify_message_records_match(
    ...
    assert!(
        !expected_data_messages.is_empty(),
        "{}: Expected data messages cannot be empty",
        test_name
    );
    assert!(
        !actual_data_messages.is_empty(),
        "{}: Actual data messages cannot be empty",
        test_name
    );
    ...
    assert_eq!(
        ...
        "{}: Schema mismatch between expected and actual messages",
        test_name
    );
    ...
    let expected_concat = concat_batches(&expected_schema, &expected_batches).expect(&format!(
        "{}: Failed to concatenate expected batches",
        test_name
    ));

    // Concatenate all actual batches into a single batch
    let actual_concat = concat_batches(&actual_schema, &actual_batches).expect(&format!(
        "{}: Failed to concatenate actual batches",
        test_name
    ));

    // Compare the concatenated batches
    assert_eq!(
        expected_concat.num_rows(),
        actual_concat.num_rows(),
    ...
    assert_eq!(
        expected_concat.num_columns(),
        actual_concat.num_columns(),
    ...
    let (expected_final, actual_final) = if preserve_order {
        ...
        let expected_indices = sort_to_indices(
            expected_concat.column(0),
            Some(SortOptions::default()),
            None,
        )
        .unwrap();
        let actual_indices =
            sort_to_indices(actual_concat.column(0), Some(SortOptions::default()), None).unwrap();

        // Create sorted batches
        let expected_sorted_columns: Vec<_> = (0..expected_concat.num_columns())
            .map(|col_idx| {
                arrow::compute::take(expected_concat.column(col_idx), &expected_indices, None)
                    .unwrap()
            })
            .collect();
        let actual_sorted_columns: Vec<_> = (0..actual_concat.num_columns())
            .map(|col_idx| {
                arrow::compute::take(actual_concat.column(col_idx), &actual_indices, None).unwrap()
            })
            .collect();

        let expected_sorted =
            RecordBatch::try_new(expected_concat.schema(), expected_sorted_columns).unwrap();
        let actual_sorted =
            RecordBatch::try_new(actual_concat.schema(), actual_sorted_columns).unwrap();
