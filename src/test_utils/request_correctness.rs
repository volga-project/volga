//! Layer C: request-mode cluster correctness (`env × profile`).
//!
//! Backend is Scylla. Windows are the first scenario. APIs are write path /
//! read path / shared state.
...
fn scylla_config(parallelism: usize) -> Result<ScyllaConfig> {
    let contact = std::env::var("VOLGA_SCYLLA_CONTACT")
        .context("VOLGA_SCYLLA_CONTACT required (host:port[,...])")?;
    Ok(ScyllaConfig {
        ...
        serving_publish: Some(crate::api::spec::state::ServingPublish::OnCommit),
        max_parallelism: Some(parallelism),
    })
}

pub async fn run_request_correctness(env: RuntimeEnv, profile: RequestProfile) -> Result<()> {
    run_request_correctness_with_fault(env, profile, RequestFault::None).await
}
...
pub async fn run_request_correctness_with_fault(
    ...
    let request_url = match env {
        ...
        RuntimeEnv::Docker | RuntimeEnv::Kube => "http://127.0.0.1:18080/request".to_string(),
    };

    // OperatorPerWorker refuses request I/O (#247). Pipelined keeps HTTP source
    // and sink on the same process (same task_index slice). Cross-process write
    // vs read placement waits on request workers.
    let (assignment, worker_count, parallelism) = (
    ...
    let cfg = scylla_config(parallelism)?;
    let pipeline = PipelineSpecBuilder::new()
        ...
        .with_operator_state_backend(OperatorStateBackendConfig::Scylla(cfg.clone()))
        .with_request_store(RequestStoreConfig::Scylla(cfg))
...
async fn post_sum(client: &reqwest::Client, url: &str, key: &str, ts: i64) -> Result<(String, f64)> {
    ...
    let row = rows.as_array().and_then(|a| a.first()).unwrap_or(rows);
    let sum = row
        .get("sum_value")
