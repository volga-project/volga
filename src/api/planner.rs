...
use datafusion::logical_expr::{Aggregate, Expr, Filter, Join, LogicalPlan, Projection, TableScan};
...
use super::logical_graph::{LogicalGraph, LogicalNode};
...
impl Planner {
    ...
    pub fn register_request_source_sink(
        &mut self,
    ...
    pub fn register_source(&mut self, table_name: String, config: SourceConfig, schema: SchemaRef) {
        self.context
            .connector_configs
            .insert(table_name.clone(), config);
                ...
                graph
                    .to_request_mode(
                        self.context
                            ...
                            .clone()
                            .expect("Request source configuration not found"),
        ...
        let mut source_config = self
            ...
            .ok_or_else(|| {
                DataFusionError::Plan(format!(
                    "No source configuration found for table '{}'",
                    table_name
                ))
            })?
            .clone();
    ...
    fn create_sink_node(
        ...
        self.logical_graph
            .add_edge(root_node_index, sink_node_index);
        Ok(())
    }
    ...
    fn handle_aggregate(&mut self, aggregate: &Aggregate, parallelism: usize) -> Result<()> {
        ...
        let physical_plan = std::thread::scope(|s| {
            let handle = s.spawn(|| {
                let rt = tokio::runtime::Runtime::new()
                    .map_err(|e| DataFusionError::External(Box::new(e)))?;
                rt.block_on(self.context.df_planner.create_physical_plan(
                    &aggregate_plan,
                    &self.context.df_session_context.state(),
                ))
            });
            handle.join().unwrap()
        })?;
        ...
        let partial_aggregate_exec =
            find_partial_aggregate(&physical_plan).expect("should have found an aggregate exec");
        // create 2 nodes - key by + aggregate node
        let key_by_node = LogicalNode::new(
            OperatorConfig::KeyByConfig(KeyByFunction::DataFusion(DataFusionKeyFunction::new(
                Arc::new(partial_aggregate_exec.clone()),
            ))),
            parallelism,
            None,
            None,
    ...
    fn handle_window(&mut self, window: &Window, parallelism: usize) -> Result<()> {
        ...
        let window_exec = physical_plan
            ...
            .expect(&format!(
                "should have found a window exec, found: {:?}",
                name
            ));
        ...
        let key_by_node = LogicalNode::new(
            OperatorConfig::KeyByConfig(KeyByFunction::DataFusion(
                DataFusionKeyFunction::new_window(Arc::new(window_exec.clone())),
            )),
            parallelism,
            None,
            None,
...
impl<'a> TreeNodeVisitor<'a> for Planner {
    ...
    fn f_down(&mut self, node: &'a Self::Node) -> Result<TreeNodeRecursion> {
        ...
        match node {
            ...
            LogicalPlan::Join(join) => self.create_join_node(join, self.context.parallelism)?,
    ...
    fn f_up(&mut self, node: &'a Self::Node) -> Result<TreeNodeRecursion> {
        ...
        if matches!(node, LogicalPlan::Aggregate(_)) {
            ...
            self.logical_graph
                .add_edge(key_by_node_index, aggregate_node_index);

            // add edge between prev node and aggregate
            let prev_node_index = self
                .node_stack
                .last()
                .expect("key by should have a node before it");
            self.logical_graph
                .add_edge(aggregate_node_index, *prev_node_index);
        ...
        if matches!(node, LogicalPlan::Window(_)) {
            ...
            // key_by -> window
            self.logical_graph
                .add_edge(key_by_node_index, window_node_index);
            ...
            let prev_node_index = self
                ...
                .expect("key by should have a node before it");
            self.logical_graph
                .add_edge(window_node_index, *prev_node_index);
...
mod tests {
    use crate::runtime::{
        functions::source::RequestSourceConfig,
        operators::source::source_operator::VectorSourceConfig, partition::PartitionType,
    };
    ...
    async fn test_simple_select_with_sink() {
        ...
        // Register sink
        planner.register_sink(SinkConfig::in_memory_grpc("http://127.0.0.1:8080"));
    ...
    async fn test_select_with_filter() {
        ...
        assert!(matches!(
            nodes[1].operator_config,
            OperatorConfig::MapConfig(MapFunction::Filter(_))
        ));
        assert!(matches!(
            nodes[2].operator_config,
            OperatorConfig::SourceConfig(_)
    ...
    async fn test_group_by() {
        ...
        assert_eq!(
            nodes.len(),
            4,
            "Expected 4 nodes (source, key_by, aggregate, projection), found {}",
            nodes.len()
        );
        ...
        assert!(matches!(
            nodes[2].operator_config,
            OperatorConfig::KeyByConfig(KeyByFunction::DataFusion(_))
        ));
        assert!(matches!(
            nodes[3].operator_config,
            OperatorConfig::SourceConfig(_)
    ...
    async fn test_group_by_multiple_clauses() {
        ...
        assert!(
            matches!(nodes[1].operator_config, OperatorConfig::AggregateConfig(_)),
            "Expected second aggregate"
        );
        assert!(
            matches!(
                nodes[2].operator_config,
        ...
        assert!(
            ...
            "Expected first projection"
        );
        assert!(
            matches!(nodes[4].operator_config, OperatorConfig::AggregateConfig(_)),
            "Expected first aggregate"
        );
        assert!(
            matches!(
                nodes[5].operator_config,
                OperatorConfig::KeyByConfig(KeyByFunction::DataFusion(_))
            ),
            "Expected first key_by"
        );
        assert!(
            matches!(nodes[6].operator_config, OperatorConfig::SourceConfig(_)),
            "Expected source"
        );
        ...
        if let OperatorConfig::AggregateConfig(config) = &nodes[4].operator_config {
            ...
            assert_eq!(
                group_exprs.len(),
                2,
                "First GROUP BY should have 2 expressions (department, role)"
            );
            assert_eq!(
                aggr_exprs.len(),
                1,
                "First aggregate should have 1 expression (AVG)"
            );
        }
        ...
        if let OperatorConfig::AggregateConfig(config) = &nodes[1].operator_config {
            ...
            assert_eq!(
                group_exprs.len(),
                1,
                "Second GROUP BY should have 1 expression (department)"
            );
            assert_eq!(
                aggr_exprs.len(),
                2,
                "Second aggregate should have 2 expressions (COUNT, SUM)"
            );
        }
    ...
    async fn test_window_query() {
        ...
        assert_eq!(
            nodes.len(),
            4,
            "Expected 4 nodes (source, key_by, window, projection), found {}",
            nodes.len()
        );
        ...
        assert!(
            ...
            "Expected final projection after window"
        );
        assert!(
            matches!(nodes[1].operator_config, OperatorConfig::WindowConfig(_)),
            "Expected window operator"
        );
        assert!(
            matches!(
                nodes[2].operator_config,
            ...
            "Expected key_by for PARTITION BY"
        );
        assert!(
            matches!(nodes[3].operator_config, OperatorConfig::SourceConfig(_)),
            "Expected source"
        );
    ...
    async fn test_window_query_to_request_mode() {
        ...
        let request_source_config = SourceConfig::HttpRequestSourceConfig(
            RequestSourceConfig::new(crate::runtime::functions::source::RequestSourceSinkSpec {
        ...
        assert_eq!(
            outgoing_after.len(),
            0,
            "Window should have no outgoing edges after conversion"
        );
    }
}
