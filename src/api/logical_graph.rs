...
mod tests {
    ...
    async fn test_linear_logical_to_execution_graph() {
        ...
        // Verify edges: source→keyby Forward 1:1 (2) + keyby→map Hash mesh (4) + map→sink Forward 1:1 (2)
        assert_eq!(graph.get_edges().len(), 8);
        ...
        for edge in graph.get_edges().values() {
            ...
            match (&source_vertex.operator_config, &target_vertex.operator_config) {
                (OperatorConfig::SourceConfig(_), OperatorConfig::KeyByConfig(_)) => {
                    assert!(matches!(edge.partition_type, crate::runtime::partition::PartitionType::Forward),
                        "Edge {} -> {} should use Forward partitioning", edge.source_vertex_id, edge.target_vertex_id);
                    assert_eq!(source_vertex.task_index, target_vertex.task_index);
                }
