...
impl LogicalNode {
    pub fn new(
        ...
        parallelism: usize,
        in_schema: Option<Arc<ArrowSchema>>,
        out_schema: Option<Arc<ArrowSchema>>,
    ) -> Self {
        Self {
...
impl LogicalGraph {
    ...
    pub fn add_node(&mut self, mut node: LogicalNode) -> NodeIndex {
        ...
        let counter = self
            .operator_type_counters
            .entry(operator_config_type.clone())
            .or_insert(0);
        *counter += 1;
    ...
    pub fn add_edge(&mut self, source: NodeIndex, target: NodeIndex) -> petgraph::graph::EdgeIndex {
        ...
        let partition_type =
            determine_partition_type(&source_node.operator_config, &target_node.operator_config);
    ...
    pub fn to_execution_graph(&self) -> ExecutionGraph {
        ...
        if self.watermarks_enabled {
            let source_nodes: Vec<NodeIndex> = self
                ...
                .filter(|&idx| {
                    matches!(
                        self.graph[idx].operator_config,
            ...
            debug_assert!(
                self.graph
                    .node_weights()
                    .all(|n| n.watermark_assign.is_none()),
        ...
        for logical_node in self.graph.node_weights() {
            ...
            for i in 0..parallelism {
                ...
                execution_vertex.watermark_assign =
                    logical_node.watermark_assign.clone().or_else(|| {
                        auto_watermark_assign
                            .get(&logical_node.operator_id)
                            .cloned()
                    });
        ...
        for edge in self.graph.edge_references() {
            ...
            let source_execution_vertices =
                &logical_to_execution_mapping[&source_logical_node_index];
            let target_execution_vertices =
                &logical_to_execution_mapping[&target_logical_node_index];
    ...
    pub fn from_linear_operators(
        operator_list: Vec<OperatorConfig>,
        parallelism: usize,
        chained: bool,
    ) -> Self {
        ...
        for i in 0..grouped_operators.len() - 1 {
            logical_graph.add_edge(node_indices[i], node_indices[i + 1]);
        }
    ...
    pub fn to_request_mode(
        ...
        for node_idx in self.graph.node_indices() {
            if matches!(
                &self.graph[node_idx].operator_config,
                OperatorConfig::WindowConfig(_)
            ) {
        ...
        assert!(
            matches!(
                &self.graph[keyby_node].operator_config,
        ...
        let keyby_node_new = LogicalNode::new(keyby_config, parallelism, None, None);
    ...
    for (i, op_config) in operators.iter().enumerate() {
        match op_config {
            OperatorConfig::ReduceConfig(_, _) => {
                ...
                if i == 0 {
                    panic!(
                        "Reduce operator '{}' requires a KeyBy operator before it",
                        op_config
                    );
                }
                ...
                match prev_config {
                    ...
                    _ => {
                        panic!(
                            "Reduce operator '{}' requires a KeyBy operator immediately before it",
                            op_config
                        );
                    }
                }
...
pub fn determine_partition_type(
    ...
    match (source_config, target_config) {
        ...
        (_, OperatorConfig::SinkConfig(SinkConfig::RequestSinkConfig)) => {
...
mod tests {
    ...
    use crate::orchestrator::task_assignment::{
        OperatorPerWorkerStrategy, TaskWorkerAssignStrategy,
    };
    ...
    fn test_logical_to_execution_graph_conversion() {
        ...
        for edge in edges.values() {
            assert!(
                ...
                "Edge {} -> {} should use RoundRobin partitioning",
                edge.source_vertex_id,
                edge.target_vertex_id
            );
            // Verify channels are not set initially
            assert!(
                edge.channel.is_none(),
                "Edge {} -> {} should not have channel set initially",
                edge.source_vertex_id,
                edge.target_vertex_id
            );
        }
    ...
    fn test_logical_to_execution_graph_with_cluster() {
        ...
        let filter_function = FilterFunction::new(
            DFSchemaRef::from(
                DFSchema::try_from(Arc::new(Schema::new(vec![Field::new(
                    "id",
                    DataType::Int32,
                    false,
                )])))
        ...
        let vertex_to_node =
            OperatorPerWorkerStrategy.assign_tasks(&execution_graph, &cluster_nodes);
        ...
        for edge in edges.values() {
            ...
            let source_node = vertex_to_node
                ...
                .expect("Source vertex should be mapped");
            let target_node = vertex_to_node
                .get(edge.target_vertex_id.as_ref())
                .expect("Target vertex should be mapped");
            ...
            if source_node.worker_id != target_node.worker_id {
                ...
                match &edge.channel {
                    ...
                    Some(Channel::Local { .. }) => {
                        panic!(
                            "Expected remote channel for cross-node edge {} -> {}",
                            edge.source_vertex_id, edge.target_vertex_id
                        );
                    }
                    None => {
                        panic!(
                            "Expected channel to be set for edge {} -> {}",
                            edge.source_vertex_id, edge.target_vertex_id
                        );
                    }
                }
    ...
    async fn test_linear_logical_to_execution_graph_chained() {
        ...
        for edge in graph.get_edges().values() {
            ...
            assert!(
                matches!(
                    edge.partition_type,
                    crate::runtime::partition::PartitionType::Hash
                ),
                "Edge {} -> {} should use Hash partitioning (keyby -> map2)",
                edge.source_vertex_id,
                edge.target_vertex_id
            );
        }
    ...
    async fn test_linear_logical_to_execution_graph() {
        ...
        for edge in graph.get_edges().values() {
            ...
            match (
                &source_vertex.operator_config,
                &target_vertex.operator_config,
            ) {
                (OperatorConfig::SourceConfig(_), OperatorConfig::KeyByConfig(_)) => {
                    ...
                    assert!(
                        matches!(
                            edge.partition_type,
                            crate::runtime::partition::PartitionType::RoundRobin
                        ),
                        "Edge {} -> {} should use RoundRobin partitioning",
                        edge.source_vertex_id,
                        edge.target_vertex_id
                ...
                (OperatorConfig::KeyByConfig(_), OperatorConfig::MapConfig(_)) => {
                    ...
                    assert!(
                        ...
                        "Edge {} -> {} should use Hash partitioning",
                        edge.source_vertex_id,
                        edge.target_vertex_id
                    );
                }
