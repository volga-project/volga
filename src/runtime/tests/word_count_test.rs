...
fn test_word_count() -> Result<()> {
    ...
    assert_eq!(
        result_vec.len(),
        parallelism,
        "Should have exactly one message per parallelism instance"
    );
    ...
    for (batch_idx, message) in result_vec.iter().enumerate() {
        ...
        assert_eq!(
            batch.num_columns(),
            2,
            "Batch {} should have 2 columns: word and count",
            batch_idx
        );
    ...
    if !failed_words.is_empty() {
        println!(
            "Words that don't match expected count of {}:",
            num_to_send_per_word
        );
        for (word, count) in &failed_words {
            println!(
                "  Word: '{}', Expected: {}, Actual: {}",
                word, num_to_send_per_word, count
            );
        }
    ...
    println!(
        "Successfully verified {} unique words, each with count {}",
        word_counts.len(),
        num_to_send_per_word
    );
