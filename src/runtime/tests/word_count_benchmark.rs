...
async fn poll_pipeline_state_updates(
    ...
    let worker_metrics = pipeline_state
        .worker_states
        .values()
        .next()
        .unwrap()
        ...
        .as_ref()
        .unwrap();
    let source_operator_metrics = worker_metrics
        .operator_metrics
        .get(&source_operator_id)
        .unwrap();
    let sink_operator_metrics = worker_metrics
        .operator_metrics
        .get(&sink_operator_id)
        .unwrap();
...
pub async fn run_word_count_benchmark(
    ...
    let source_operator_id = logical_graph
        .get_nodes_by_predicate(|node| {
            matches!(node.operator_config, OperatorConfig::SourceConfig(_))
        })
        .first()
        .unwrap()
        .operator_id
        .clone();
    let sink_operator_id = logical_graph
        .get_nodes_by_predicate(|node| {
            matches!(node.operator_config, OperatorConfig::SinkConfig(_))
        })
        .first()
        .unwrap()
        .operator_id
    ...
    for (batch_idx, message) in result_vec.iter().enumerate() {
        ...
        if batch.num_columns() != 2 {
            println!(
                "Warning: Batch {} has {} columns instead of 2",
                batch_idx,
                batch.num_columns()
            );
            continue;
    ...
    let word_counts: HashMap<String, f64> = word_counts
        .into_iter()
        .map(|(k, v)| (k, v as f64))
        .collect();
...
async fn test_word_count_benchmark() -> Result<()> {
    ...
    println!(
        "\nSample Count: {}",
        benchmark_metrics.messages_per_second.len()
    );
    println!("=====================================\n");
    ...
    assert!(
        !benchmark_metrics.messages_per_second.is_empty(),
        "Should have collected some throughput metrics"
    );
    assert!(
        !benchmark_metrics.latency_histogram.is_some(),
