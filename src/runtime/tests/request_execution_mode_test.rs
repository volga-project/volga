...
fn create_payload_generator(
    ...
    for task_index in 0..parallelism {
        let task_keys = DatagenSourceFunction::gen_key_values_for_task(
            parallelism,
            task_index,
            num_unique_keys,
        );
...
async fn test_request_execution_mode() {
    ...
    request_source_sink_spec.schema_json = Some(schema_to_json(schema.as_ref()));
    ...
    wait_for_request_server(
        &request_source_config.spec.bind_address,
    ...
    let payload_generator =
        create_payload_generator(parallelism, num_unique_keys, start_ms, step_ms);
    ...
    let mut responses_by_key: HashMap<String, Vec<(String, f64, i64, f64, f64, f64)>> =
        HashMap::new();
                    ...
                    match response_data {
                        Some(data_array) => {
                            if !data_array.is_empty() {
                                ...
                                assert!(
                                    first_result.get("sum_value").is_some(),
                                    "Response should contain sum_value"
                                );
                                assert!(
                                    first_result.get("count_value").is_some(),
                                    "Response should contain count_value"
                                );
                                assert!(
                                    first_result.get("avg_value").is_some(),
                                    "Response should contain avg_value"
                                );
                                assert!(
                                    first_result.get("min_value").is_some(),
                                    "Response should contain min_value"
                                );
                                assert!(
                                    first_result.get("max_value").is_some(),
                                    "Response should contain max_value"
                                );

                                successful_requests += 1;

                                // Extract key and aggregates for tracking
                                let event_time = first_result
                                    .get("event_time")
                                    .and_then(|v| v.as_str())
                                    .unwrap();
                                let key = first_result
                                    .get("key")
                                    .and_then(|v| v.as_str())
                                    .unwrap()
                                    .to_string();
                                let sum = first_result
                                    .get("sum_value")
                                    .and_then(|v| v.as_f64())
                                    .unwrap();
                                let count = first_result
                                    .get("count_value")
                                    .and_then(|v| v.as_i64())
                                    .unwrap();
                                let avg = first_result
                                    .get("avg_value")
                                    .and_then(|v| v.as_f64())
                                    .unwrap();
                                let min = first_result
                                    .get("min_value")
                                    .and_then(|v| v.as_f64())
                                    .unwrap();
                                let max = first_result
                                    .get("max_value")
                                    .and_then(|v| v.as_f64())
                                    .unwrap();
                        ...
                        None => {
                            println!(
                                "❌ Request {}: Invalid response structure",
                                result.request_id
                            );
                            failed_requests += 1;
                        }
