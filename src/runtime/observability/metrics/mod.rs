...
impl OperatorAggregateMetrics {
    pub fn new(operator_id: String, task_metrics: Vec<TaskMetrics>) -> Self {
        let latency_stats =
            LatencyMetrics::merge(task_metrics.iter().map(|m| &m.latency_stats).collect());
...
impl WorkerAggregateMetrics {
    pub fn new(
        ...
        tasks_metrics: HashMap<String, TaskMetrics>,
...
pub fn emit_storage_stats_gauges(
    snapshot: &StorageStatsSnapshot,
    labels: &MetricsLabels,
    storage_type: &str,
) {
    let base = || (LABEL_STORAGE_TYPE, storage_type.to_string());
...
pub struct PipelineStateHistory {
    pub samples: Vec<(u64, crate::runtime::observability::PipelineSnapshot)>,
}
...
impl PipelineStateHistory {
    ...
    pub fn add_sample(
        &mut self,
        timestamp: u64,
        pipeline_state: crate::runtime::observability::PipelineSnapshot,
    ) {
        self.samples.push((timestamp, pipeline_state));
    }

    pub fn calculate_throughput_rates(
        &self,
        window_seconds: u64,
    ) -> HashMap<String, ThroughputRates> {
                ...
                if let Some(start_worker_state) = start_state.worker_states.get(worker_id) {
                    if let Some(start_worker_metrics) = &start_worker_state.worker_metrics {
                        for (vertex_id, end_task_metrics) in &end_worker_metrics.tasks_metrics {
                            if let Some(start_task_metrics) =
                                start_worker_metrics.tasks_metrics.get(vertex_id)
                            {
                                let messages_sent_diff = end_task_metrics
                                    .throughput_stast
                                    .messages_sent
                                    .saturating_sub(
                                        start_task_metrics.throughput_stast.messages_sent,
                                    );
                                let messages_recv_diff = end_task_metrics
                                    .throughput_stast
                                    .messages_recv
                                    .saturating_sub(
                                        start_task_metrics.throughput_stast.messages_recv,
                                    );
                                let records_sent_diff = end_task_metrics
                                    .throughput_stast
                                    .records_sent
                                    .saturating_sub(
                                        start_task_metrics.throughput_stast.records_sent,
                                    );
                                let records_recv_diff = end_task_metrics
                                    .throughput_stast
                                    .records_recv
                                    .saturating_sub(
                                        start_task_metrics.throughput_stast.records_recv,
                                    );
                                let bytes_sent_diff = end_task_metrics
                                    .throughput_stast
                                    .bytes_sent
                                    .saturating_sub(start_task_metrics.throughput_stast.bytes_sent);
                                let bytes_recv_diff = end_task_metrics
                                    .throughput_stast
                                    .bytes_recv
                                ...
                                rates.insert(
                                    vertex_id.clone(),
                                    ThroughputRates {
                                        messages_sent_per_sec: messages_sent_diff as f64
                                            / time_diff_seconds,
                                        messages_recv_per_sec: messages_recv_diff as f64
                                            / time_diff_seconds,
                                        records_sent_per_sec: records_sent_diff as f64
                                            / time_diff_seconds,
                                        records_recv_per_sec: records_recv_diff as f64
                                            / time_diff_seconds,
                                        bytes_sent_per_sec: bytes_sent_diff as f64
                                            / time_diff_seconds,
                                        bytes_recv_per_sec: bytes_recv_diff as f64
                                            / time_diff_seconds,
                                    },
                                );
                            }
    ...
    pub fn final_stats(&self) -> FinalStats {
                            ...
                            for (vertex_id, end_task_metrics) in &end_worker_metrics.tasks_metrics {
                                    ...
                                    start_worker_metrics.tasks_metrics.get(vertex_id)
                                {
                                    ...
                                    let bytes_sent_diff = end_task_metrics
                                        ...
                                        .bytes_sent
                                        .saturating_sub(
                                            start_task_metrics.throughput_stast.bytes_sent,
                                        );
                                    let bytes_recv_diff = end_task_metrics
                                        .throughput_stast
                                        .bytes_recv
                                        .saturating_sub(
                                            start_task_metrics.throughput_stast.bytes_recv,
                                        );
        ...
        for (vertex_id, rates_vec) in all_rates_per_task {
            ...
            let messages_sent_values: Vec<f64> =
                rates_vec.iter().map(|r| r.messages_sent_per_sec).collect();
            let messages_recv_values: Vec<f64> =
                rates_vec.iter().map(|r| r.messages_recv_per_sec).collect();
            let records_sent_values: Vec<f64> =
                rates_vec.iter().map(|r| r.records_sent_per_sec).collect();
            let records_recv_values: Vec<f64> =
                rates_vec.iter().map(|r| r.records_recv_per_sec).collect();
            let bytes_sent_values: Vec<f64> =
                rates_vec.iter().map(|r| r.bytes_sent_per_sec).collect();
            let bytes_recv_values: Vec<f64> =
                rates_vec.iter().map(|r| r.bytes_recv_per_sec).collect();

            throughput_per_task.insert(
                vertex_id.clone(),
                TaskThroughputStats {
...
fn _init_metrics() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        !LATENCY_BUCKET_BOUNDARIES.contains(&f64::MAX),
...
fn parse_stream_task_metrics(
    ...
    let throughput_stats = ThroughputMetrics::new(
        messages_sent,
        messages_recv,
        records_sent,
        records_recv,
...
mod tests {
    ...
    fn test_stream_task_metrics_prometheus_parsing() {
        ...
        let expected_histogram =
            build_expected_histogram(&latency_measurements, &LATENCY_BUCKET_BOUNDARIES);

        let parsed_metrics =
            get_stream_task_metrics(Arc::<str>::from(vertex_id.clone()), Some(&labels));
        ...
        assert!(
            !parsed_metrics.latency_stats.latency_histogram.is_empty(),
        ...
        for (i, (&expected, &actual)) in expected_histogram
            .iter()
            .zip(parsed_metrics.latency_stats.latency_histogram.iter())
            .enumerate()
        {
            assert_eq!(
                actual, expected,
        ...
        let total_count = parsed_metrics
            .latency_stats
            .latency_histogram
            .last()
            .unwrap_or(&0);
        assert_eq!(
            *total_count,
            latency_measurements.len() as u64,
            "Should have recorded {} latency measurements",
