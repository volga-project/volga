...
use crate::common::message::{BaseMessage, KeyedMessage, Message};
...
impl KeyByFunctionTrait for ArrowKeyByFunction {
    fn key_by(&self, message: Message) -> Vec<KeyedMessage> {
        ...
        for i in 0..record_batch.num_columns() {
            let sorted_array =
                compute::take(record_batch.column(i).as_ref(), &sorted_indices, None).unwrap();
...
impl KeyByFunctionTrait for DataFusionKeyFunction {
    ...
    fn key_by(&self, message: Message) -> Vec<KeyedMessage> {
        ...
        for (_hash, indices) in hash_to_indices {
            ...
            let group_columns: Vec<_> = record_batch
                ...
                .map(|col| {
                    compute::take(col.as_ref(), &indices_array, None)
                        .expect("should be able to take values for group")
                })
                .collect();

            let group_batch = RecordBatch::try_new(record_batch.schema(), group_columns)
                .expect("should be able to create batch for group");
            ...
            let key_batch = RecordBatch::try_new(key_schema, key_arrays)
                .expect("should be able to creat key batch");
...
impl FunctionTrait for KeyByFunction {
    async fn open(&mut self, _context: &RuntimeContext) -> Result<()> {
        match self {
            ...
            KeyByFunction::DataFusion(function) => {
                function.runtime_context = Some(_context.clone())
            }
        }
        Ok(())
    }
...
mod tests {
    ...
    fn test_arrow_key_by_single_column() {
        ...
        for keyed_message in &keyed_messages {
            ...
            assert!(
                id_col.iter().all(|val| val == Some(id)),
                "Not all IDs in batch are equal: {:?}",
                id_col
            );
    ...
    fn test_arrow_key_by_multiple_columns() {
        ...
        let key_by_function =
            ArrowKeyByFunction::new(vec!["category".to_string(), "subcategory".to_string()]);
        ...
        for keyed_message in &keyed_messages {
            ...
            let values: Vec<f64> = (0..batch.num_rows()).map(|i| value_col.value(i)).collect();
    ...
    async fn create_test_setup() -> (Planner, Arc<Schema>, Message) {
        ...
        let name_array =
            StringArray::from(vec!["alice", "bob", "alice", "charlie", "alice", "bob"]);
        let dept_array = StringArray::from(vec!["eng", "sales", "sales", "eng", "eng", "eng"]);
        let salary_array = Int32Array::from(vec![80000, 50000, 60000, 90000, 85000, 75000]);
    ...
    async fn test_datafusion_key_by_aggregate_source() {
        ...
        for node in &nodes {
            if let crate::runtime::operators::operator::OperatorConfig::KeyByConfig(
                key_by_function,
...
pub async fn extract_datafusion_window_exec(
    sql: &str,
    planner: &mut Planner,
) -> Arc<BoundedWindowAggExec> {
    let logical_graph = planner.sql_to_graph(sql).unwrap();
    let nodes: Vec<_> = logical_graph.get_nodes().collect();
