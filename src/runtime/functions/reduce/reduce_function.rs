...
impl AggregationResultExtractorTrait for AggregationResultExtractor {
    fn extract_result(
        ...
        match self {
            AggregationResultExtractor::All(e) => {
                e.extract_result(key, result, upstream_vertex_id, ingest_timestamp)
            }
            AggregationResultExtractor::Single(e) => {
                e.extract_result(key, result, upstream_vertex_id, ingest_timestamp)
            }
            AggregationResultExtractor::Custom(e) => {
                e.extract_result(key, result, upstream_vertex_id, ingest_timestamp)
            }
        }
    }
...
impl AggregationResultExtractor {
    ...
    pub fn single_aggregation(aggregation_type: AggregationType, field_name: String) -> Self {
        Self::Single(SingleAggregationResultExtractor::new(
            aggregation_type,
            field_name,
        ))
    }
...
mod tests {
    ...
    fn create_test_keyed_batch(values: Vec<f64>, key_value: i32) -> KeyedMessage {
        ...
        let key_schema = Arc::new(arrow::datatypes::Schema::new(vec![Field::new(
            "key",
            DataType::Int32,
            false,
        )]));
        ...
        let key_batch = RecordBatch::try_new(key_schema, vec![Arc::new(key_array)]).unwrap();
    ...
    async fn test_arrow_reduce_function() {
        ...
        let min_extractor =
            SingleAggregationResultExtractor::new(AggregationType::Min, "min_value".to_string());
        let min_message = min_extractor.extract_result(&key, &result, None, None);
        let min_value = min_message
            .record_batch()
            .column(0)
            .as_any()
        ...
        let max_extractor =
            SingleAggregationResultExtractor::new(AggregationType::Max, "max_value".to_string());
        let max_message = max_extractor.extract_result(&key, &result, None, None);
        let max_value = max_message
            .record_batch()
            .column(0)
            .as_any()
            .downcast_ref::<Float64Array>()
        ...
        let sum_extractor =
            SingleAggregationResultExtractor::new(AggregationType::Sum, "sum_value".to_string());
        let sum_message = sum_extractor.extract_result(&key, &result, None, None);
        let sum_value = sum_message
            .record_batch()
            .column(0)
            .as_any()
            .downcast_ref::<Float64Array>()
    ...
    async fn test_arrow_reduce_function_different_types() {
        ...
        let string_schema = Arc::new(Schema::new(vec![Field::new(
            "value",
            DataType::Utf8,
            false,
        )]));
        ...
        let string_batch =
            RecordBatch::try_new(string_schema.clone(), vec![Arc::new(string_array)]).unwrap();
