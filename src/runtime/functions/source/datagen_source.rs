...
use arrow::array::builder::{
    Float64Builder, Int64Builder, StringBuilder, TimestampMillisecondBuilder,
};
...
impl DatagenSourceFunction {
    ...
    pub(crate) fn init_keys(&mut self) {
        ...
        if let Some(key_idx) = key_field_index {
            ...
            if let Some(FieldGenerator::Key { num_unique }) =
                self.config.spec.fields.get(key_field_name)
            {
            ...
            self.per_key_increments
                .insert(global_key.clone(), HashMap::new());
            self.per_key_values_indices
                .insert(global_key, HashMap::new());
        }
    }
    ...
    fn build_batch(&mut self, batch_size: usize) -> Result<RecordBatch> {
        ...
        for (field_idx, field) in self.schema.fields().iter().enumerate() {
            ...
            let column = match generator {
                ...
                FieldGenerator::Key { .. } => Self::generate_key_column_static(&batch_keys)?,

                FieldGenerator::Increment { start, step } => {
                    Self::generate_increment_column_per_key(
                        &mut self.per_key_increments,
                        field_idx,
                        start,
                        step,
                        &batch_keys,
                ...
                FieldGenerator::Values { values } => Self::generate_values_column_per_key(
                    &mut self.per_key_values_indices,
                    field_idx,
                    values,
                    &batch_keys,
                )?,
    ...
    fn generate_random_string_column_static(
        length: usize,
        batch_size: usize,
        rng: &mut StdRng,
    ) -> Result<ArrayRef> {
    ...
    fn generate_increment_column_per_key(
        per_key_increments: &mut HashMap<String, HashMap<usize, ScalarValue>>,
        field_idx: usize,
        start: &ScalarValue,
        step: &ScalarValue,
        batch_keys: &[String],
        ...
        match (start, step) {
            (ScalarValue::Int64(Some(start_val)), ScalarValue::Int64(Some(step_val))) => {
                ...
                for key in batch_keys {
                    ...
                    let current_val = key_increments
                        .entry(field_idx)
            ...
            _ => Err(anyhow::anyhow!(
                "Unsupported increment types: {:?} and {:?}",
                start,
                step
            )),
        }
    ...
    fn generate_uniform_column_static(
        min: &ScalarValue,
        max: &ScalarValue,
        batch_size: usize,
        rng: &mut StdRng,
    ...
    fn generate_values_column_per_key(
        per_key_values_indices: &mut HashMap<String, HashMap<usize, usize>>,
        field_idx: usize,
        values: &[ScalarValue],
        batch_keys: &[String],
    ) -> Result<ArrayRef> {
        ...
        match &values[0] {
            ...
            ScalarValue::Utf8(_) => {
                let mut builder =
                    StringBuilder::with_capacity(batch_keys.len(), batch_keys.len() * 20);
                for key in batch_keys {
                    let key_values_indices = per_key_values_indices.get_mut(key).unwrap();
                    let current_index = key_values_indices.entry(field_idx).or_insert(0);
...
mod tests {
        ...
        for (key, key_records) in records_by_key {
            ...
            for record in &key_records {
                if let Some(prev) = prev_timestamp {
                    assert!(
                        record.0 > prev,
                        "Timestamps should be incremental for key {}",
                        key
                    );
                }
            ...
            for record in &key_records {
                if let Some(prev) = prev_id {
                    assert_eq!(
                        record.2,
                        prev + 1,
                        "IDs should increment by 1 for key {}",
                        key
                    );
                }
            ...
            for record in &key_records {
                assert!(
                    record.3 >= 0.0 && record.3 <= 100.0,
                    "Value should be in range [0, 100] for key {}",
                    key
                );
            }
            ...
            for record in &key_records {
                assert!(
                    ["A", "B", "C"].contains(&record.4.as_str()),
                    "Category should be A, B, or C for key {}",
                    key
                );
            }
        }
    ...
    async fn test_datagen_rate_timing() {
        ...
        assert_eq!(
            all_timestamps.len(),
            40,
            "Should generate exactly 40 records"
        );
    ...
    fn create_replayable_checkpoint_test_config(
        total_records: usize,
        batch_size: usize,
    ...
    async fn collect_rows(
        source: &mut DatagenSourceFunction,
        ...
        if let Some(n) = target_rows {
            assert_eq!(
                out.len(),
                n,
                "test expects aligned checkpoint after full batches"
            );
        }
        out
    }
