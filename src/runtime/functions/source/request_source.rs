use super::json_utils::{json_to_record_batch, record_batch_to_json};
use anyhow::Result;
...
impl RequestSourceProcessor {
    ...
    async fn process_single_response(
        ...
        let request_id = response
            .get_extras()
            .and_then(|extras| extras.get(SOURCE_REQUEST_ID_FIELD).cloned())
            .ok_or_else(|| {
                anyhow::anyhow!("Response missing {} in extras", SOURCE_REQUEST_ID_FIELD)
            })?;
        ...
        if let Some(mut pending_entry) = pending_requests.get_mut(&request_id) {
            ...
            if pending_entry.record_count > pending_entry.expected_record_count {
                panic!(
                    "Received more records than expected for request {}",
                    request_id
                );
            }
            if pending_entry.record_count == pending_entry.expected_record_count {
        ...
        if should_send_response {
            ...
            let (_, pending_response) = pending_requests
                .remove(&request_id)
            ...
            let result_batch =
                concat_batches(&schema, &batches).expect("Failed to concatenate record batches");
            let response_data = record_batch_to_json(&result_batch)
                .expect("Failed to convert record batch to JSON");
...
impl HttpRequestSourceFunction {
    ...
    fn create_message_from_request(
        &self,
        request_id: &str,
        ...
        let task_index = self
            .runtime_context
            .as_ref()
            .expect("Runtime context not set")
            .task_index();
        ...
        let payload_array = match &payload.data {
            ...
            _ => {
                return Err(anyhow::anyhow!(
                    "Expected request data to be an object with 'payload' field"
                ))
            }
        };
...
impl FunctionTrait for HttpRequestSourceFunction {
    async fn open(&mut self, context: &RuntimeContext) -> Result<()> {
        ...
        self.shared_request_rx = Some(
            context
                .get_request_sink_source_request_receiver()
                .expect("request_sink_source_request_receiver should be set"),
        );
        Ok(())
    }
    ...
    async fn fetch(&mut self, _interrupt: Option<&SourceInterrupt>) -> FetchResult {
        if let Some(shared_rx) = &self.shared_request_rx {
            ...
            if let Some(pending_request) = pending_request {
                match self.create_message_from_request(
...
mod tests {
    ...
    async fn create_test_processor_and_source(
        schema: SchemaRef,
    ) -> (RequestSourceProcessor, HttpRequestSourceFunction) {
    ...
    async fn test_request_fetch_response() {
        ...
        runtime_context.set_request_sink_source_request_receiver(
            processor.get_shared_request_receiver().clone(),
        );
        ...
        let name_column_index = schema
            .column_with_name("name")
            .expect("Should have name column")
            .0;
        let name_column = record_batch
            .column(name_column_index)
            .as_any()
        ...
        let age_column_index = schema
            .column_with_name("age")
            .expect("Should have age column")
            .0;
        let age_column = record_batch
            .column(age_column_index)
            .as_any()
    ...
    async fn test_backpressure() {
        ...
        assert_eq!(
            ...
            "Should have exactly max_pending requests pending"
        );
            ...
            match future.now_or_never() {
                Some(Ok(Ok(response))) => {
                    ...
                    assert_eq!(
                        ...
                        "Request {} completed immediately but with unexpected status: {}",
                        i,
                        response.status()
                    );
        ...
        assert_eq!(
            still_pending, max_pending,
            "Number of pending requests should equal max_pending capacity"
        );
        assert_eq!(
            too_many_requests_count + still_pending,
            total_requests,
