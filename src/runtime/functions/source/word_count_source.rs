                        ...
                        if sent_count < num_to_send {
                            ...
                            *self.copies_sent_per_word.entry(word.clone()).or_insert(0) += to_add;
                            ...
                            if sent_count + to_add >= num_to_send {
                                self.current_index =
                                    (self.current_index + 1) % self.dictionary.len();
                            }
            ...
            BatchingMode::RoundRobin => {
                ...
                if let Some(num_to_send) = self.num_to_send_per_word {
                    ...
                    loop {
                        ...
                        let all_sent = self
                            .copies_sent_per_word
                            .values()
                            .all(|count| *count >= num_to_send);
                        if all_sent {
...
async fn test_word_count_source(
    ...
    while let Some(message) = source.fetch(None).await.into_message() {
        match message {
                ...
                if batching_mode == BatchingMode::SameWord {
                    ...
                    for i in 1..word_array.len() {
                        assert_eq!(
                            word_array.value(i),
                            first_word,
                            "All words in batch should be identical"
                        );
                    }
                }
            ...
            Message::Watermark(_) => {
                panic!("WordCountSourceFunction should not emit watermarks directly")
            }
    ...
    if is_fixed_size {
        ...
        assert!(
            word_counts.len() <= dictionary_size,
            "Should not have sent more unique words than dictionary size"
        );
        ...
        assert!(
            elapsed_time >= run_for_s.unwrap(),
            "Source should have run for at least {} seconds, but ran for {}",
            run_for_s.unwrap(),
            elapsed_time
        );
        assert!(
            elapsed_time <= run_for_s.unwrap() + 1,
            "Source should not have run much longer than {} seconds, but ran for {}",
            run_for_s.unwrap(),
            elapsed_time
        );
    }
