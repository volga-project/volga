...
impl ProjectionFunction {
    pub fn new(
        ...
        let physical_exprs = exprs
            ...
            .map(|e| {
                session_context
                    .create_physical_expr(e.clone(), &in_schema)
                    .expect("unable to create physical expr")
            })
            .collect::<Vec<_>>();
...
impl MapFunctionTrait for ProjectionFunction {
    fn map(&self, message: Message) -> Result<Message> {
        ...
        let projected_batch =
            RecordBatch::try_new(self.out_schema.inner().clone(), projected_arrays)
...
mod tests {
    ...
    async fn test_projection_function() {
        ...
        let expressions = vec![logical_expr::col("value"), logical_expr::col("name")];
        ...
        let projected_value_array = projected_batch
            .column(0)
            .as_any()
            .downcast_ref::<Int32Array>()
            .unwrap();
        let projected_name_array = projected_batch
            .column(1)
            .as_any()
        ...
        assert_eq!(
            projected_name_array.iter().collect::<Vec<_>>(),
    ...
    async fn test_projection_with_literal() {
        ...
        let ctx = SessionContext::new();
        let expressions = vec![logical_expr::col("value"), logical_expr::lit(42)];
        ...
        let projected_value_array = projected_batch
            ...
            .downcast_ref::<Int32Array>()
            .unwrap();
        let literal_array = projected_batch
            .column(1)
            .as_any()
            .downcast_ref::<Int32Array>()
            .unwrap();
