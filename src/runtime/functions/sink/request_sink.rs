...
use crate::runtime::functions::source::request_source::{
    SOURCE_REQUEST_ID_FIELD, SOURCE_TASK_INDEX_FIELD,
};
    ...
    fn validate_message(&self, message: &Message) -> Result<()> {
        ...
        if extras.get(SOURCE_TASK_INDEX_FIELD).is_none() {
            return Err(anyhow::anyhow!(
                "Message missing {} in extras",
                SOURCE_TASK_INDEX_FIELD
            ));
        }
...
impl FunctionTrait for RequestSinkFunction {
    async fn open(&mut self, context: &RuntimeContext) -> Result<()> {
        ...
        let response_sender = context
            .get_request_sink_source_response_sender()
            .expect("request_sink_source_response_sender should be set");
