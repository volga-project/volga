...
impl OperatorTrait for WindowOperator {
    async fn open(&mut self, context: &RuntimeContext) -> Result<()> {
        ...
        if self.state.is_none() {
            ...
            anyhow::ensure!(
                parallelism >= 1,
                "window operator parallelism must be >= 1, got {parallelism}"
            );
            ...
            let key_group_range = KeyGroupRange::for_subtask(
                task_index as usize,
            ...
            let scope = WindowStoreTaskScope {
                ...
                attempt,
                request_mode: self.output_mode == WindowOutputMode::StateOnly
                    && context.request_store().is_some(),
            };
