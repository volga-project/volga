use crate::{
    common::{
        test_utils::{create_test_string_batch, gen_unique_grpc_port},
        Message, WatermarkMessage, MAX_WATERMARK_VALUE,
    },
    runtime::{
        functions::{
...
impl MapFunctionTrait for AddPrefixMapFunction {
    fn map(&self, message: Message) -> Result<Message> {
        ...
        let output_array: StringArray = input_array
            .iter()
...
async fn test_chained_operator_map_map() {
    let configs = vec![
        OperatorConfig::MapConfig(MapFunction::new_custom(AddPrefixMapFunction {
...
async fn test_chained_operator_source_map() {
    ...
    let test_messages = vec![
        Message::new(
            None,
            create_test_string_batch(vec!["hello".to_string()]),
            None,
            None,
        ),
        Message::new(
            None,
            create_test_string_batch(vec!["world".to_string()]),
            None,
            None,
        ),
        Message::Watermark(WatermarkMessage::new(
            "source".to_string(),
            MAX_WATERMARK_VALUE,
            None,
        )),
    ...
    let configs = vec![
        OperatorConfig::SourceConfig(SourceConfig::VectorSourceConfig(VectorSourceConfig::new(
            test_messages,
        ))),
        OperatorConfig::MapConfig(MapFunction::new_custom(ToUpperCaseMapFunction)),
    ];
...
async fn test_chained_operator_source_keyby() {
    ...
    let batch =
        arrow::record_batch::RecordBatch::try_new(schema, vec![Arc::new(values), Arc::new(keys)])
            .unwrap();
...
async fn test_chained_operator_source_map_keyby() {
    ...
    let batch = arrow::record_batch::RecordBatch::try_new(schema, vec![Arc::new(values)]).unwrap();
...
async fn test_chained_operator_type() {
    ...
    let source_sink_chained =
        ChainedOperator::new(OperatorConfig::ChainedConfig(source_sink_configs));
    assert_eq!(
        source_sink_chained.operator_type(),
...
async fn test_chained_operator_source_map_sink() {
    ...
    assert_eq!(
        chained_operator.operator_type(),
        OperatorType::ChainedSourceSink
    );
    ...
    let stored_array1 = stored_messages[0]
        .record_batch()
        .column(0)
        .as_any()
        .downcast_ref::<StringArray>()
        .unwrap();
    let stored_array2 = stored_messages[1]
        .record_batch()
        .column(0)
        .as_any()
        .downcast_ref::<StringArray>()
