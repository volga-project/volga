...
impl AggregateOperator {
    ...
    fn emit_all_accumulators(&mut self) -> Option<Message> {
        ...
        for (_key, (first_message, accumulators)) in accumulators {
            ...
            if let Some(word_array) = group_arrays[0]
                .as_any()
        ...
        println!(
            "EMIT: vertex_id={} emitting {} words: {:?}",
            vertex_id,
            words_being_emitted.len(),
            words_being_emitted
        );
                ...
                match message {
                    Message::Keyed(keyed_message) => {
                        ...
                        if !self.accumulators.contains_key(&key.hash()) {
                            ...
                            self.accumulators
                                .insert(key.hash(), (keyed_message.base.clone(), accs));
                        }
                        let (_first_message, accumulators) = self
                            .accumulators
                            .get_mut(&key.hash())
                        ...
                        for (i, accumulator) in accumulators.iter_mut().enumerate() {
                            if let Some(aggr_expr) = aggregate_exec.aggr_expr().get(i) {
                                let values = aggr_expr
                                    ...
                                    .iter()
                                    .map(|expr| {
                                        expr.evaluate(&keyed_message.base.record_batch)
                                            .expect("should be able to evaluate expression")
                                    })
                                    .collect::<Vec<_>>();
                                ...
                                let arrays: Vec<ArrayRef> = values
                                    ...
                                    .filter_map(|v| match v {
                                        ...
                                        ColumnarValue::Scalar(s) => {
                                            ...
                                            let batch_size =
                                                keyed_message.base.record_batch.num_rows();
...
mod tests {
    ...
    async fn test_aggregate_operator() {
        ...
        use crate::common::{BaseMessage, Key, KeyedMessage, MAX_WATERMARK_VALUE};
        ...
        for node in &nodes {
            if let OperatorConfig::AggregateConfig(config) = &node.operator_config {
                aggregate_operator = Some(AggregateOperator::new(OperatorConfig::AggregateConfig(
                    config.clone(),
                )));
                break;
            }
        ...
        assert_eq!(
            final_result.len(),
            1,
            "Should produce exactly one message with concatenated results"
        );
        ...
        assert_eq!(
            batch.num_columns(),
            6,
            "Should have 6 columns: name, count, sum_value, avg_value, max_value, min_value"
        );
        assert_eq!(
            batch.num_rows(),
            3,
            "Should have 3 rows (one for each group: alice, bob, charlie)"
        );
        ...
        let count_column = batch
            ...
            .unwrap();
        let sum_column = batch
            .column(2)
            .as_any()
            .downcast_ref::<Int64Array>()
            .unwrap();
        let avg_column = batch
            .column(3)
            .as_any()
            .downcast_ref::<Float64Array>()
            .unwrap();
        let max_column = batch
            .column(4)
            .as_any()
            .downcast_ref::<Int64Array>()
            .unwrap();
        let min_column = batch
            .column(5)
            .as_any()
            .downcast_ref::<Int64Array>()
            .unwrap();
        ...
        assert_eq!(
            alice_results.1, 60,
            "Alice should have sum of 60 (10+20+30)"
        );
        assert!(
            (alice_results.2 - 20.0).abs() < 1e-10,
            "Alice should have avg of 20.0, got {}",
            alice_results.2
        );
        ...
        assert!(
            (bob_results.2 - 45.0).abs() < 1e-10,
            "Bob should have avg of 45.0, got {}",
            bob_results.2
        );
        ...
        assert!(
            (charlie_results.2 - 60.0).abs() < 1e-10,
            "Charlie should have avg of 60.0, got {}",
            charlie_results.2
        );
