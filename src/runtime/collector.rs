...
impl Collector {
    pub fn new(data_writer: DataWriter, partition: Partition) -> Self {
        Self {
            data_writer,
    ...
    pub fn gen_partitioned_channels(&mut self, message: &Message) -> Vec<Channel> {
        ...
        partitions
            .iter()
            .map(|partition_idx| self.output_channels[*partition_idx].clone())
            .collect()
    }
    ...
    pub async fn write_message_to_operators(
        ...
        for (operator_id, collector) in collectors.iter_mut() {
            ...
            futures.push(async move {
                (
                    operator_id,
                    collector.write_message_to_channels(message, channels).await,
                )
            });
        }
