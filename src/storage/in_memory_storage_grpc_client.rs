...
impl InMemoryStorageClient {
    ...
    pub async fn new(addr: String) -> Result<Self> {
        ...
        for attempt in 0..MAX_RETRIES {
            match InMemoryStorageServiceClient::connect(addr.clone()).await {
                Ok(client) => {
                    println!(
                        "[IN_MEMORY_STORAGE_CLIENT] Successfully connected to {} on attempt {}",
                        addr,
                        attempt + 1
                    );
                    let client = client
                ...
                Err(e) => {
                    ...
                    println!(
                        "[IN_MEMORY_STORAGE_CLIENT] Connection attempt {} failed: {}",
                        attempt + 1,
                        e
                    );
    ...
    pub async fn insert_keyed_many(
        &mut self,
        ...
        let request = tonic::Request::new(InsertKeyedManyRequest {
            keyed_messages: keyed_messages_bytes,
        });
