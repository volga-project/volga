...
pub async fn stream_grpc_many_clients_one_server() {
    ...
    let server_handle = tokio::spawn(async move {
        start_server(server_addr.to_string(), server_tx, shutdown_rx)
            .await
            .unwrap();
    });
    ...
    for (client_name, channel_id, message_count) in &client_configs {
        println!(
            "[TEST] Starting client: {} on channel: {} with {} messages",
        ...
        let client_handle = tokio::spawn(async move {
            start_client(format!("http://{}", server_addr), client_rx)
                .await
                .unwrap();
        });
    ...
    for (client_name, channel_id, message_count, client_tx) in client_senders {
        let send_task = tokio::spawn(async move {
            ...
            for i in 0..message_count {
                ...
                println!(
                    "[TEST] Client {} sending message {} to channel {}",
                    client_name,
                    i + 1,
                    channel_id
                );
    ...
    while let Some((message, channel_id)) = server_rx.recv().await {
        ...
        println!(
            "[TEST] Received message {} on channel: {}",
            total_received, channel_id
        );
        ...
        match &message {
            Message::Regular(base_msg) => {
                println!(
                    "[TEST] Regular message with {} rows",
                    base_msg.record_batch.num_rows()
                );
            }
            Message::Keyed(keyed_msg) => {
                println!(
                    "[TEST] Keyed message with key hash: {}",
                    keyed_msg.key.hash()
                );
            }
            Message::Watermark(watermark_msg) => {
                println!(
                    "[TEST] Watermark message: {} -> {}",
                    watermark_msg.metadata.upstream_vertex_id.as_ref().unwrap(),
                    watermark_msg.watermark_value
                );
            }
        ...
        if total_received >= total_expected {
            println!(
                "[TEST] Received all expected messages ({}), stopping",
                total_expected
            );
            break;
        }
    ...
    for (channel_id, sent_messages_for_channel) in &sent_messages {
        ...
        let received_count = received_messages
            .get(channel_id)
            .map(|v| v.len())
            .unwrap_or(0);
        println!(
            "[TEST] Channel {}: expected {}, received {}",
            channel_id, expected_count, received_count
        );
        assert_eq!(
            received_count, expected_count,
            "Message count mismatch for channel {}",
            channel_id
        );
    }
    ...
    for (channel_id, sent_messages_for_channel) in &sent_messages {
        let received_messages_for_channel = received_messages
            .get(channel_id)
            .expect(&format!("No received messages for channel {}", channel_id));

        println!(
            "[TEST] Verifying channel {}: {} messages",
            channel_id,
            sent_messages_for_channel.len()
        );
        ...
        for (i, (_, sent_message)) in sent_messages_for_channel.iter().enumerate() {
            ...
            println!(
                "[TEST] Verifying channel {} message {}: {:?} type",
                channel_id,
                i + 1,
            ...
            assert_eq!(
                get_message_type(sent_message),
                get_message_type(received_message),
                "Message type mismatch for channel {} message {}",
                channel_id,
                i + 1
            );
            ...
            match (sent_message, received_message) {
                (Message::Regular(sent), Message::Regular(received)) => {
                    assert_eq!(
                        sent.record_batch.num_rows(),
                        received.record_batch.num_rows(),
                        "Row count mismatch for channel {} message {}",
                        channel_id,
                        i + 1
                    );
                    assert_eq!(
                        sent_message.upstream_vertex_id(),
                        received_message.upstream_vertex_id(),
                        "Upstream vertex ID mismatch for channel {} message {}",
                        channel_id,
                        i + 1
                    );
                }
                (Message::Keyed(sent), Message::Keyed(received)) => {
                    assert_eq!(
                        sent.key.hash(),
                        received.key.hash(),
                        "Key hash mismatch for channel {} message {}",
                        channel_id,
                        i + 1
                    );
                    assert_eq!(
                        sent.base.record_batch.num_rows(),
                        received.base.record_batch.num_rows(),
                        "Row count mismatch for channel {} message {}",
                        channel_id,
                        i + 1
                    );
                ...
                (Message::Watermark(sent), Message::Watermark(received)) => {
                    assert_eq!(
                        sent.metadata.upstream_vertex_id,
                        received.metadata.upstream_vertex_id,
                        "Source vertex ID mismatch for channel {} message {}",
                        channel_id,
                        i + 1
                    );
                    assert_eq!(
                        sent.watermark_value,
                        received.watermark_value,
                        "Watermark value mismatch for channel {} message {}",
                        channel_id,
                        i + 1
                    );
                }
            ...
            println!(
                "[TEST] Channel {} message {} verification passed",
                channel_id,
                i + 1
            );
        }
