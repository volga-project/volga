...
impl Batcher {
    pub fn new(config: BatcherConfig, senders: HashMap<String, mpsc::Sender<Message>>) -> Self {
        ...
        let mut keyed_queues: HashMap<String, Arc<Mutex<HashMap<u64, Arc<Mutex<Vec<Message>>>>>>> =
            HashMap::new();
    ...
    pub async fn write_message(
        ...
        match &message {
            Message::Keyed(keyed_msg) => {
                ...
                return Self::handle_queue_with_message(
                    message,
                    keyed_queue_guard.as_mut(),
                    sender,
                    self.config.batch_size,
                    self.current_memory_bytes.clone(),
    ...
    async fn batch_and_flush(
        ...
        while queue.len() != 0 {
            ...
            let messages_to_batch: Vec<Message> =
                queue.drain(batch_start..=(queue.len() - 1)).collect();
    ...
    fn batch_messages(messages: Vec<Message>) -> Message {
        ...
        let record_batches: Vec<arrow::record_batch::RecordBatch> =
            messages.iter().map(|m| m.record_batch().clone()).collect();

        let batched_record_batch = concat_batches(&record_batches[0].schema(), &record_batches)
            .expect("Failed to concat record batches");

        let first_message = &messages[0];

        // ingest_timestamp of a batch should be max among all
        let ingest_timestamp = messages
            .iter()
            .map(|m| m.ingest_timestamp().unwrap())
            .max()
            .unwrap();
        // TODO merge extras
        ...
        let batched_message = match first_message {
            ...
            Message::Keyed(_) => {
                ...
                let key = first_message
                    .key()
                    .expect("Keyed message should have key")
                    .clone();
                Message::new_keyed(
                    first_message.upstream_vertex_id().clone(),
                    batched_record_batch,
    ...
    async fn sync_memory_tracker(&mut self) {
        ...
        for queue in self.regular_queues.values() {
            ...
            actual_memory += queue_guard
                .iter()
                .map(|m| m.get_memory_size())
                .sum::<usize>();
        }
        ...
        self.current_memory_bytes
            .store(actual_memory, std::sync::atomic::Ordering::Relaxed);
    }
...
mod tests {
    ...
    fn create_test_keyed_message(rows: usize, timestamp: u64, key: String) -> Message {
        ...
        let key_batch = arrow::record_batch::RecordBatch::try_new(
            key_schema,
            vec![Arc::new(StringArray::from(vec![key.clone()]))],
        )
        .unwrap();
    ...
    fn test_concat_messages() {
        ...
        match batched {
            Message::Keyed(keyed_msg) => {
                ...
                assert_eq!(keyed_msg.base.metadata.ingest_timestamp, Some(101));
                // Max timestamp
            }
    ...
    async fn test_batcher_watermark_passthrough() {
        ...
        batcher
            .write_message(&"test_channel".to_string(), watermark)
            .await
            .unwrap();
    ...
    async fn test_batcher_regular_batching() {
        ...
        for i in 0..3 {
            ...
            batcher
                .write_message(&"test_channel".to_string(), msg)
                .await
                .unwrap();
        }
    ...
    async fn test_memory_tracking_accuracy() {
        ...
        let tracked_memory = batcher
            .current_memory_bytes
            .load(std::sync::atomic::Ordering::Relaxed);
        assert_eq!(
            tracked_memory, expected_memory,
    ...
    async fn test_watermark() {
        ...
        for i in 0..3 {
            let msg = create_test_message(2, 100 + i);
            batcher
                .write_message(&"regular_channel".to_string(), msg)
                .await
                .unwrap();
        }
        ...
        for i in 0..3 {
            ...
            batcher
                .write_message(&"keyed_channel".to_string(), msg)
                .await
                .unwrap();
        }
        ...
        for i in 0..2 {
            ...
            batcher
                .write_message(&"mixed_channel".to_string(), regular_msg)
                .await
                .unwrap();

            let keyed_msg = create_test_keyed_message(2, 400 + i, "key2".to_string());
            batcher
                .write_message(&"mixed_channel".to_string(), keyed_msg)
                .await
                .unwrap();
        }
        ...
        assert!(
            rx_regular.try_recv().is_err(),
            "Regular channel should not have auto-flushed"
        );
        assert!(
            rx_keyed.try_recv().is_err(),
            "Keyed channel should not have auto-flushed"
        );
        assert!(
            rx_mixed.try_recv().is_err(),
            "Mixed channel should not have auto-flushed"
        );

        let watermark_regular = Message::Watermark(crate::common::message::WatermarkMessage::new(
            "watermark_regular".to_string(),
            1000,
            Some(1000),
        ));
        batcher
            .write_message(&"regular_channel".to_string(), watermark_regular)
            .await
            .unwrap();

        let watermark_keyed = Message::Watermark(crate::common::message::WatermarkMessage::new(
            "watermark_keyed".to_string(),
            2000,
            Some(2000),
        ));
        batcher
            .write_message(&"keyed_channel".to_string(), watermark_keyed)
            .await
            .unwrap();

        let watermark_mixed = Message::Watermark(crate::common::message::WatermarkMessage::new(
            "watermark_mixed".to_string(),
            3000,
            Some(3000),
        ));
        batcher
            .write_message(&"mixed_channel".to_string(), watermark_mixed)
            .await
            .unwrap();
        ...
        assert!(
            rx_regular.try_recv().is_err(),
            "Regular channel should have no more messages"
        );
        assert!(
            rx_keyed.try_recv().is_err(),
            "Keyed channel should have no more messages"
        );
        assert!(
            rx_mixed.try_recv().is_err(),
            "Mixed channel should have no more messages"
        );
    }
    ...
    async fn test_batcher_e2e(
        ...
        println!(
            "[DEBUG] Test config: duration={}s, channels={:?}, keys_per_channel={}, batch_size={}",
        ...
        let receiver_handle = {
            ...
            tokio::spawn(async move {
                ...
                let mut channel_messages_map: HashMap<String, HashMap<String, Vec<Message>>> =
                    HashMap::new();
                ...
                for (channel_id, mut rx) in receivers {
                    ...
                    let task = tokio::spawn(async move {
                        ...
                        while let Some(msg) = rx.recv().await {
                            ...
                            if msg_count % 100 == 0 {
                                println!(
                                    "[DEBUG] Channel {} received {} messages",
                                    channel_id_clone, msg_count
                                );
                            }
                        ...
                        println!(
                            "[DEBUG] Channel {} receiver finished, total messages: {}",
                            channel_id_clone, msg_count
                        );
            ...
            tokio::spawn(async move {
                ...
                while start_time.elapsed() < Duration::from_secs(test_duration_secs) {
                    for channel_id in &channels {
                        ...
                        batcher_clone
                            .write_message(channel_id, regular_msg)
                            .await
                            .unwrap();
                        total_sent += 1;
                        message_id += 1;
                        ...
                        // Send to batcher immediately
                        batcher_clone
                            .write_message(channel_id, keyed_msg)
                            .await
                            .unwrap();
                        total_sent += 1;
                        message_id += 1;
                        ...
                        if message_id % 10 == 0 {
                            let watermark =
                                Message::Watermark(crate::common::message::WatermarkMessage::new(
                            ...
                            batcher_clone
                                .write_message(channel_id, watermark)
                                .await
                                .unwrap();
                            total_sent += 1;
                            message_id += 1;
                ...
                println!(
                    "[DEBUG] Message generation finished, total sent: {}",
                    total_sent
                );
        ...
        for channel_id in &channels {
            ...
            for (i, (sent_wm, received_wm)) in sent_watermarks
                .iter()
                .zip(received_watermarks.iter())
                .enumerate()
            {
                match (sent_wm, received_wm) {
                    (Message::Watermark(sent), Message::Watermark(received)) => {
                        assert_eq!(
                            sent.watermark_value, received.watermark_value,
                            "Watermark {} value mismatch",
                            i
                        );
                    }
            ...
            for (key, sent_msgs) in sent_messages {
                ...
                println!(
                    "Channel {} Key {}: Sent {} messages, Received {} batched messages",
                    channel_id,
                    key,
                    sent_msgs.len(),
                    received_msgs.len()
                ...
                while sent_msg_index < sent_msgs.len() && received_msg_index < received_msgs.len() {
                    ...
                    let sent_rows = match sent_msg {
                        ...
                        Message::CheckpointBarrier(_) => {
                            panic!("Unexpected checkpoint barrier in sent messages")
                        }
                    };
                    ...
                    let received_rows = match received_msg {
                        ...
                        Message::CheckpointBarrier(_) => {
                            panic!("Unexpected checkpoint barrier in batched messages")
                        }
                    };

                    let sent_ids = sent_rows
                        .column(0)
                        .as_any()
                        .downcast_ref::<Int64Array>()
                        .unwrap();
                    let sent_data = sent_rows
                        .column(1)
                        .as_any()
                        .downcast_ref::<StringArray>()
                        .unwrap();
                    let received_ids = received_rows
                        .column(0)
                        .as_any()
                        .downcast_ref::<Int64Array>()
                        .unwrap();
                    let received_data = received_rows
                        .column(1)
                        .as_any()
                        .downcast_ref::<StringArray>()
                        .unwrap();
        ...
        println!(
            ...
            final_sent_messages
                .values()
                .map(|channel_msgs| channel_msgs.values().map(|msgs| msgs.len()).sum::<usize>())
                .sum::<usize>()
        );
        println!(
            "Total watermarks sent: {}",
            final_sent_watermarks
                .values()
                .map(|msgs| msgs.len())
                .sum::<usize>()
        );
        println!(
            "Total batched messages received: {}",
            final_received_messages
                .values()
                .map(|channel_msgs| channel_msgs.values().map(|msgs| msgs.len()).sum::<usize>())
                .sum::<usize>()
        );
        println!(
            "Total watermarks received: {}",
            final_received_watermarks
                .values()
                .map(|msgs| msgs.len())
                .sum::<usize>()
        );
