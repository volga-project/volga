    ...
    F: Fn(usize) -> serde_json::Value,
{
    ...
    let mut interval_opt =
        requests_per_second.map(|rps| interval(Duration::from_millis((1000.0 / rps) as u64)));
            ...
            let result = match response_result {
                Ok(response) => {
                    ...
                    match serde_json::from_str::<serde_json::Value>(&text) {
                        ...
                        Err(e) => Err(format!(
                            "Failed to parse response JSON, response: {:?}, status: {}, error: {}",
                            text, status, e
                        )),
                    }
                }
        ...
        Ok((status, response_body)) => {
            if *status != 200 {
                return Err(format!(
                    ...
                    request_result.request_id, status
                ));
            }

            // Extract request payload data
            let request_data = request_result
                .request_payload
                .get("data")
                ...
                .ok_or_else(|| {
                    format!(
                        "Request {} has invalid payload structure",
            ...
            let response_data = response_body
                ...
                .ok_or_else(|| {
                    format!(
                        "Request {} response has invalid structure",
                        request_result.request_id
                    )
                })?;
            ...
            for (key, request_record) in &request_records {
                let response_record = response_records.get(key).ok_or_else(|| {
                    format!(
                        "Request {} missing record in response: {}",
                        request_result.request_id, key
                    )
                })?;

                // Verify field values match
                for field in fields_to_verify {
                    let request_value = request_record.get(field).ok_or_else(|| {
                        format!(
                            "Request {} missing field '{}' in request",
                            request_result.request_id, field
                        )
                    })?;
                    let response_value = response_record.get(field).ok_or_else(|| {
                        format!(
                            "Request {} missing field '{}' in response",
                            request_result.request_id, field
                        )
                    })?;
...
async fn test_request_source_sink_e2e() {
    ...
    planner.register_source(
        ...
        SourceConfig::VectorSourceConfig(
            crate::runtime::operators::source::source_operator::VectorSourceConfig::new(vec![]),
        ),
        schema.clone(),
    );
    ...
    let mut executor = RequestExecutor::start(
        ...
        RequestExecutorOptions {
            ...
            worker_id: "request-executor".to_string(),
            wro_store: None,
    ...
    println!(
        "📊 Completed {} requests, analyzing results...",
        results.len()
    );
    ...
    let duration_ms: Vec<f64> = all_durations
        .iter()
    ...
    println!(
        "  Success Rate: {:.1}%",
        (successful_requests as f64 / results.len() as f64) * 100.0
    );
    ...
    assert_eq!(
        results.len(),
        total_requests,
        "Should complete all requested requests"
    );
    assert_eq!(
