...
use crate::runtime::operators::window::state::index::bucket_index::BatchMeta;
...
pub struct Compactor {
    ...
    bucket_locks: DashMap<(u64, Timestamp), Arc<Mutex<()>>>,
    ...
    last_dumped_version: DashMap<(u64, Timestamp), u64>,
}
...
impl Compactor {
        ...
        let compact_handle = tokio::spawn(async move {
            ...
            loop {
                ...
                let _ = compactor_for_compact
                    .periodic_compact_in_mem(&ws_for_compact, ts_column_index, hot_bucket_count)
                    .await;
            }
        });
        ...
        let dump_handle = tokio::spawn(async move {
            ...
            loop {
                ...
                let _ = compactor_for_dump
                    .periodic_dump_to_store(&ws_for_dump, ts_column_index, hot_bucket_count)
                    .await;
            }
        });
    ...
    fn get_bucket_lock(&self, key_hash: u64, bucket_ts: Timestamp) -> Arc<Mutex<()>> {
        let k = (key_hash, bucket_ts);
    ...
    async fn _rehydrate_bucket(
        ...
        let _lease = self.batch_pins.clone().pin(stored_ids.clone());
        let loaded = self.batch_store.load_batches(stored_ids, key).await;
    ...
    async fn dump_bucket(
        ...
        for seg in segments {
            ...
            self.batch_store.put_batch_with_id(new_id, seg.clone(), key).await;
        ...
        if new_metas.is_empty() {
            self.retired.retire(key, &new_stored_ids);
            return Ok(false);
        }
        ...
        if !prev_persisted.is_empty() || !pending_stored.is_empty() {
            ...
            self.retired.retire(key, &all_vec);
        }
    ...
    pub async fn periodic_dump_to_store(
        ...
        for entry in window_states.iter() {
            ...
            for (bucket_ts, expected_version) in to_dump {
                let lock = self.get_bucket_lock(key_hash, bucket_ts);
                let _guard = lock.lock().await;

                let should_dump = {
                    let state = arc.read().await;
                    match state.bucket_index.bucket(bucket_ts) {
                        None => false,
                        Some(bucket) => bucket.version == expected_version,
                    }
                };
                if !should_dump {
                    continue;
                ...
                if published {
                    self.last_dumped_version
                        .insert((key_hash, bucket_ts), expected_version);
                }
            }
        }
        Ok(())
    ...
    pub async fn checkpoint_flush_to_store(
        ...
        for entry in window_states.iter() {
            let key = entry.key().clone();
            let arc = entry.value().clone();
            let bucket_ts_list: Vec<Timestamp> = {
                let guard = arc
                    .try_read()
                    .expect("Failed to read windows state for flush planning");
                guard.bucket_index.bucket_timestamps()
            };
