...
#[derive(Debug)]
pub struct InMemoryStorageGrpcSinkFunction {
...
impl InMemoryStorageGrpcSinkFunction {
...
#[async_trait]
impl SinkFunctionTrait for InMemoryStorageGrpcSinkFunction {
...
#[async_trait]
impl FunctionTrait for InMemoryStorageGrpcSinkFunction {
    async fn open(&mut self, context: &RuntimeContext) -> Result<()> {
        ...
        self.flush_handle = Some(tokio::spawn(async move {
            ...
            while running.load(Ordering::SeqCst) {
                interval.tick().await;
                if let Err(e) = InMemoryStorageGrpcSinkFunction::flush_buffers(&shared_client, &buffer, &keyed_buffer, Some(&vertex_id)).await {
...
mod tests {
    ...
    fn test_in_memory_storage_grpc_sink_function() -> Result<()> {
        ...
        runtime.block_on(async {
            ...
            storage_server.start("127.0.0.1:50060").await?;
            
            // Wait a bit for server to start
            tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;

            // Create sink function
            let mut sink_function = InMemoryStorageGrpcSinkFunction::new("http://127.0.0.1:50060".to_string());
            ...
            let mut client = InMemoryStorageClient::new("http://127.0.0.1:50060".to_string()).await?;
