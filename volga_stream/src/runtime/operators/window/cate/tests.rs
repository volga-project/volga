...
#[tokio::test]
async fn test_top_n_key_sum_cate_where_orders_by_key() {
    let sql = r#"SELECT
    timestamp,
    value,
    partition_key,
    top_n_key_sum_cate_where(value, value > 0, partition_key, 2) OVER w as top_val
FROM test_table
WINDOW w AS (
  PARTITION BY partition_key
  ORDER BY timestamp
  RANGE BETWEEN INTERVAL '1000' MILLISECOND PRECEDING AND CURRENT ROW
    ...
    acc.update_batch(&args).expect("update");
    let out = acc.evaluate().expect("evaluate");
    assert_eq!(out, ScalarValue::Utf8(Some("c:2,b:3".to_string())));
}

#[tokio::test]
async fn test_top_n_value_sum_cate_where_orders_by_value() {
    let sql = r#"SELECT
    timestamp,
    value,
    partition_key,
    top_n_value_sum_cate_where(value, value > 0, partition_key, 2) OVER w as top_val
FROM test_table
WINDOW w AS (
  PARTITION BY partition_key
  ORDER BY timestamp
  RANGE BETWEEN INTERVAL '1000' MILLISECOND PRECEDING AND CURRENT ROW
    ...
    acc.update_batch(&args).expect("update");
    let out = acc.evaluate().expect("evaluate");
    assert_eq!(out, ScalarValue::Utf8(Some("c:5,b:3".to_string())));
}
