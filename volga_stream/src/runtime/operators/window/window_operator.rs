...
impl WindowOperatorConfig {
    ...
    pub fn set_spec(&mut self, spec: WindowOperatorSpec) {
        self.spec = spec;
        if let Some(v) = &self.spec.tiling {
            let n = self.window_exec.window_expr().len().max(1);
            if self.tiling_configs.is_empty() {
                self.tiling_configs = vec![Some(v.clone()); n];
            } else {
                for slot in self.tiling_configs.iter_mut() {
                    *slot = Some(v.clone());
                }
            }
        }
...
mod tests {
    ...
    async fn test_window_operator_checkpoint_consistency() {
        ...
        window_config.spec.parallelize = false;
    ...
    async fn test_pruning_with_lateness() {
        ...
        let mut window_config = WindowOperatorConfig::new(window_exec);
        window_config.spec.parallelize = true;
        window_config.spec.lateness = Some(3000); // 3 seconds lateness
