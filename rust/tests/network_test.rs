...
fn test_one_to_one() {
    ...
    let mut to_send = vec![];
    for i in 0..num_msgs {
        to_send.push(Box::new(
            bincode::serialize(&ChannelMessage{key: i.to_string(), value: random_string(payload_size)}).unwrap().to_vec()
        ));
    }
    ...
    let j_handle = std::thread::spawn(move|| {
        ...
        for msg in local_to_send.as_ref() {
            backp += l_w.write_bytes(channel.get_channel_id(), msg.clone(), 1000, 0).unwrap();
        }
        backp
    });
    ...
    // TODO assert order and equality
