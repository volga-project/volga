...
use super::{channel::Channel, data_reader::{DataReader, DataReaderConfig}, data_writer::{DataWriter, DataWriterConfig}, io_loop::{IOHandler, IOLoop, ZmqConfig}, remote_transfer_handler::{RemoteTransferHandler, TransferConfig}};
...
impl PyDataReader {
    ...
    #[new]
    pub fn new(handler_id: String, name: String, job_name: String, config: &DataReaderConfig, channels: Vec<&PyAny>) -> PyDataReader {
        ...
        let data_reader = DataReader::new(handler_id, name, job_name, config.clone(), rust_channels);
    ...
    pub fn stop(&self) {
        (self.data_reader.clone() as Arc<dyn IOHandler>).stop();
    }
...
impl PyDataWriter {
    ...
    #[new]
    pub fn new(handler_id: String, name: String, job_name: String, config: &DataWriterConfig, channels: Vec<&PyAny>) -> PyDataWriter {
        let mut rust_channels = Vec::new();
        for ch in channels {
            let ext: Result<PyLocalChannel, pyo3::PyErr> = ch.extract();
            if ext.is_ok() {
        ...
        let data_writer = DataWriter::new(handler_id, name, job_name, config.clone(), rust_channels);
...
impl PyTransferSender {
    #[new]
    pub fn new(handler_id: String, name: String, job_name: String, config: &TransferConfig, channels: Vec<PyRemoteChannel>) -> PyTransferSender {
        let mut rust_channels = Vec::new();
        for ch in channels {
            rust_channels.push(ch.to_rust_channel());
        };
        let transfer_sender = RemoteTransferHandler::new(handler_id, name, job_name, rust_channels, config.clone(), true);
...
impl PyTransferReceiver {
    #[new]
    pub fn new(handler_id: String, name: String, job_name: String, config: &TransferConfig, channels: Vec<PyRemoteChannel>) -> PyTransferReceiver {
        let mut rust_channels = Vec::new();
        for ch in channels {
            rust_channels.push(ch.to_rust_channel());
        };
        let transfer_receiver = RemoteTransferHandler::new(handler_id, name, job_name, rust_channels, config.clone(), false);
...
impl PyIOLoop {
    ...
    pub fn connect(&self, timeout_ms: u128) -> Option<String> {
        self.io_loop.connect(timeout_ms)
    }
