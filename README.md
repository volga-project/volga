...
Benchmarks are streaming mode only, and only a continuously sliding window query. Request mode, batch, and other SQL shapes are not covered.
...
- [ ] Backfills
- [ ] Regular aggregations (`GROUP BY` - streaming + request + split optimizer)
- [ ] Joins (streaming / temporal + request / lookup + split optimizer)
- [ ] Generic split optimizer for arbitrary SQL
- [ ] Python client
- [ ] UI
